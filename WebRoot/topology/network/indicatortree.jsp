				...
				if(isShowTreeNodeFlag){
			    	...
			    	// 不干任何事
						...
						L3:for(Object nodeDTOObject : nodeDTOList){
						    ...
						    d.add(currTreeNodeId,treeNodeFatherId,"<%=treeFlag.equals("1")?nodeDTO.getName().trim():nodeDTO.getIpaddress()%>","",'<%=nodeDTO.getIpaddress()%>',"<%=currTreeNode.getNodeTag()+nodeDTO.getId()+";"+currTreeNode.getName()%>","",imagestr,imagestr);
							<%
							//yangjun add
							treeNodeNum++;
					        ...
					        L4:for(Object nodeObject : alarmlist){
							    ...
							    var namestr = "<input type='checkbox' name='check' value='<%=alarmnode.getId()+":"+alarmnode.getNodeid()%>' <%=checked%>><%=alarmnode.getName()%>";
