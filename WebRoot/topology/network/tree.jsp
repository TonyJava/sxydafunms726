					...
					if(isShowTreeNodeFlag){
				    	...
				    	// 不干任何事
							...
							L3:for(Object nodeDTOObject : nodeDTOList){
							    ...
							    d.add(currTreeNodeId,currTreeNodeFatherId,'<%=treeFlag.equals("0")?nodeDTO.getName().trim():nodeDTO.getIpaddress()%>',"",'<%=nodeDTO.getIpaddress()%>',"<%=currTreeNode.getNodeTag()+nodeDTO.getId()+";"+currTreeNode.getName()%>","",imagestr);
								<%
					   		} // 完成 每一个设备 循环 (L3)
					   	}
					}
