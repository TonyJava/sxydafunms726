	...
	<head>
		...
		<script language="JavaScript">
			...
			*/
			...
			function addmanage()
			{
				mainForm.action ="<%=rootPath%>/service.do?action=changeManage&value=1&id="+ node + "&type=" + category + "&category=" + "<%=category%>";
				mainForm.submit();
			}
			
			function cancelmanage()
			{
				mainForm.action ="<%=rootPath%>/service.do?action=changeManage&value=0&id="+ node + "&type=" + category + "&category=" + "<%=category%>";
				mainForm.submit();
											...
											<td>
												<table id="content-header" class="content-header">
								                	<tr>
									                	...
									                	<td class="content-title"> 应用 &gt;&gt; 服务监控 &gt;&gt; 服务监控列表</td>
		        									        ...
		        									        for(int i = 0 ; i < list.size() ; i++){
		        									            ...
		        									            <tr <%=onmouseoverstyle%>>
		        									            	<td align="center" class="body-data-list"><%=i+1%></td>
			       													...
			       													<td align="center" class="body-data-list">
			       														<img src="<%=rootPath%>/resource/image/status.gif" border="0" width=15 oncontextmenu=showMenu('2','<%=monitorServiceDTO.getId()%>','<%=monitorServiceDTO.getIpAddress()%>','<%=monitorServiceDTO.getCategory()%>') alt="右键操作">
			       													</td>
					        									</tr>
