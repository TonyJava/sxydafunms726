...
<html>
	...
	*/		
	function toAdd(){
		mainForm.action = "<%=rootPath%>/resourceConfigNode.do?action=add&id=<%=agentid%>";
		mainForm.submit();
	}
	function toDelete(){
		mainForm.action = "<%=rootPath%>/resourceConfigNode.do?action=delete&id=<%=agentid%>";
		mainForm.submit();
	}
</script>
		<script language="JavaScript" type="text/JavaScript">
											...
											<td>
												<table id="content-header" class="content-header">
								                	<tr>
									                	...
									                	<td class="content-title"> 系统管理 >> 维护期设置 >> 设置列表 >> 资源选择 </td>
																...
																<%
																	if(list != null && list.size() > 0) {
																		for(int i = 0; i < list.size(); i++){
																			ResourceConfNode rnode=(ResourceConfNode)list.get(i);
																%>
																<tr>
																	<td align="center" class="body-data-list">
																		<INPUT type="checkbox" value="<%=rnode.getNodeid()%>" name="checkbox" onclick="">
																		<%=i+1%>
																	</td>
																	<td align="center" class="body-data-list">
																		<%=rnode.getIp_address() %>
