...
function parseDataForTree()
{
	...
	function loadIds(id,ids,startdate,todate,busiId){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=executeReport&id="+id+"&ids="+ids+"&startdate="+startdate+"&todate="+todate+"&business="+busiId+"&nowtime="+(new Date()),
...
Ext.onReady(function(){
	...
	function handleActivate(tab){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=loadCompreReportDayList&type=hostNet&reportType=busi&nowtime="+(new Date()),
			success:function(data){
...
<script>
	$(document).ready(function(){
		$('#saveBtn').bind('click',function(){
        	...
        	$.ajax({
				type:"POST",
				data:"ids="+ids+"&tile="+tile+"&desc="+desc+"&exporttype="+exporttype+"&report_name="+report_name+"&recievers_name="+name+"&sendtimehou="+hou+"&recievers_id="+re_id+"&business="+business+"&nowtime="+(new Date()),
				dataType:"json",
				url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=saveCompreReportDayOption&type=hostNet&reportType=busi",
				success:function(data){
					alert(data.dataStr);
			 		var tab = Ext.getCmp('ext-tab-report');
           			tab.setActiveTab(1);	
...
function deleteItem(id){
	if(window.confirm("您确定要删除吗？")){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=loadCompreReportDayList&type=hostNet&id="+id+"&reportType=busi&nowtime="+(new Date()),
			success:function(data){
				var modelDiv =document.getElementById("model");
				modelDiv.innerHTML=data.dataStr;
			}
...
function createWin(id){
	return CreateDeviceWindow("<%=rootPath%>/comprereportbusi.do?action=compreReportDayConfig&id="+id);
}
</script>
								...
								<div id='tabs1'>
									<div id="script" class="x-hide-display">
                            			<table border="0" cellpadding="0" cellspacing="0" class="win-content" id="win-content">
											...
											<tr>
                                            	<td width=100% >
                                               		<jsp:include page="../busireport/compreSubscribeAddBusi.jsp"></jsp:include>
                                            	</td>
                                    		</tr>
                                    		<tr>
                                    			<td width="94%">
