...
function parseDataForTree()
{
	...
	function loadIds(id,ids,startdate,todate){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=executeReportWeek&id="+id+"&ids="+ids+"&startdate="+startdate+"&todate="+todate+"&nowtime="+(new Date()),
...
Ext.onReady(function(){
	...
	function handleActivate(tab){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=loadCompreReportDayList&type=hostNet&reportType=week&nowtime="+(new Date()),
			success:function(data){
...
<script>
	$(document).ready(function(){
		$('#saveBtn').bind('click',function(){
        	...
        	$.ajax({
				type:"POST",
				data:"ids="+ids+"&tile="+tile+"&desc="+desc+"&exporttype="+exporttype+"&report_name="+report_name+"&recievers_name="+name+"&sendtimehou="+hou+"&sendtimeweek="+week+"&recievers_id="+re_id+"&reportType=week&nowtime="+(new Date()),
				dataType:"json",
				url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=saveCompreReportDayOption&type=hostNet",
				success:function(data){
					alert(data.dataStr);
			 		var tab = Ext.getCmp('ext-tab-report');
           			tab.setActiveTab(1);	
...
function deleteItem(id){
	if(window.confirm("您确定要删除吗？")){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportAjaxManager.ajax?action=loadCompreReportDayList&type=hostNet&id="+id+"&reportType=week&nowtime="+(new Date()),
			success:function(data){
				var modelDiv =document.getElementById("model");
				modelDiv.innerHTML=data.dataStr;
			}
...
function exportReport(exportType){
	...
	window.open('<%=rootPath%>/comprereportweek.do?action=downloadReportWeek&type=hostNet&reportType=week&exportType='+exportType+'&id='+id+'&ids='+ids+'&startdate='+startdate+'&todate='+todate+'&nowtime='+(new Date()),"_blank","toolbar=no,width=1,height=1,top=2000,left=3000,directories=no,status=no,menubar=no,alwaysLowered=yes");
}
								...
								<div id='tabs1'>
									<div id="script" class="x-hide-display">
                            			<table border="0" cellpadding="0" cellspacing="0" class="win-content" id="win-content">
											...
											<tr>
                                            	<td width="94%" >
                                               		<jsp:include page="../busireport/compreSubscribeAddWeek.jsp"></jsp:include>
                                            	</td>
                                    		</tr>
                                    		<tr>
                                    			<td width="94%">
