...
<%
  ...
  String startdate =(String)request.getAttribute("startTime");
  String todate = (String)request.getAttribute("toTime");
%>
<html>
	<head>
		...
		//	alert("ids:"+ids);
		  ...
		  document.all.ids_perform.value=ids;
		  document.all.id_perform.value=id;
		 	//alert("ids:"+ids+"       id:"+id +   "startdate:"+startdate   +"todate"+todate);
		 loadIds(id,ids,startdate,todate);
		}
		</script>
...
var	ids = ddtree.getAllChecked().toString();
   ...
   $.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportWeekAjaxManager.ajax?action=loadPeforIndex&type=host&nowtime="+(new Date()),
			success:function(data){
...
//模板创建信息保存
	$(document).ready(function(){
		...
		var eventAnalyze = $('#eventAnalyze').val();
		var hostTopAnalyze = $('#hostTopAnalyze').val();
		var netTopAnalyze = $('#netTopAnalyze').val();
			if($('#eventCount').val()==0 && $('#hostAlarmCount').val()==0 && $('#hostTopAnalyze').val()==0 && $('#netTopAnalyze').val()==0)
				return;
		//jhl end
	...
	if(ids.length<=0|| ids.length == "")
	{
	   ...
	   return;
        ...
        $.ajax({
			...
			"&eventAnalyze="+eventAnalyze+
			"&hostTopAnalyze="+hostTopAnalyze+
			"&netTopAnalyze="+netTopAnalyze),
			dataType:"json",
			url:"<%=rootPath%>/compreReportWeekAjaxManager.ajax?action=savePerforIndexWeek&type=host",
			success:function(data){
...
function deleteItem(id){
 if(window.confirm("您确定要删除吗？")){
  $.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/compreReportWeekAjaxManager.ajax?action=loadPeforIndex&type=host&id="+id+"&nowtime="+(new Date()),
			success:function(data){
...
return CreateDeviceWindow("<%=rootPath%>/compreWeekCapReportManager.do?action=netHostWorkReportConfig&id="+id);
...
if((ids.length<=0|| ids.length == "")&&(id.length<=0|| id.length == "")){
 ...
 window.open('<%=rootPath%>/compreWeekCapReportManager.do?action=downloadReportWeek&type=hostNet&exportType='+exportType+'&id='+id+'&ids='+ids+'&startdate='+startdate+'&todate='+todate+'&nowtime='+(new Date()),"_blank","toolbar=no,width=1,height=1,top=2000,left=3000,directories=no,status=no,menubar=no,alwaysLowered=yes");
...
<tr>
																...
																<tr>
                                                                      <td width=100% >
                                                                         <jsp:include page="subscribeAdd.jsp"></jsp:include>
                                                                      </td>
                                                                  </tr>
																<tr>
