...
function parseDataForTree()
{
	...
	function loadIds(id,ids,terms,startdate,todate){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/linkReportAjaxManager.ajax?action=executeReport&id="+id+"&terms="+terms+"&ids="+ids+"&startdate="+startdate+"&todate="+todate+"&nowtime="+(new Date()),
			success:function(data){
				if(data.upChart == 0){ 
					...
					so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/link_report_up_settings.xml"));
				...
				if(data.downChart == 0){ 
					...
					var so = new SWFObject("<%=rootPath%>/amchart/amcolumn.swf", "ampie","885", "338", "8", "#FFFFFF");
					so.addVariable("path", "<%=rootPath%>/amchart/");
					so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/link_report_down_settings.xml"));
				...
				if(data.bandwidthChart == 0){ 
					...
					var so = new SWFObject("<%=rootPath%>/amchart/amcolumn.swf", "ampie","885", "338", "8", "#FFFFFF");
					so.addVariable("path", "<%=rootPath%>/amchart/");
					so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/link_report_bandwidth_settings.xml"));
				...
				if(data.bandtrendChart == 0){ 
					...
					var so = new SWFObject("<%=rootPath%>/amchart/amcolumn.swf", "ampie","885", "338", "8", "#FFFFFF");
					so.addVariable("path", "<%=rootPath%>/amchart/");
					so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/link_report_bandtrend_settings.xml"));
				...
				if(data.usabilityChart == 0){ 
					...
					var so = new SWFObject("<%=rootPath%>/amchart/amcolumn.swf", "ampie","885", "338", "8", "#FFFFFF");
					so.addVariable("path", "<%=rootPath%>/amchart/");
					so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/link_report_usability_settings.xml"));
...
Ext.onReady(function(){
	...
	function handleActivate(tab){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/linkReportAjaxManager.ajax?action=loadLinkReportList&type=day&nowtime="+(new Date()),
			success:function(data){
...
<script>
	$(document).ready(function(){
		$('#saveBtn').bind('click',function(){
        	...
        	$.ajax({
				type:"POST",
				data:"ids="+ids+"&tile="+tile+"&desc="+desc+"&exporttype="+exporttype+"&report_name="+report_name+"&recievers_name="+name+"&sendtimehou="+hou+"&recievers_id="+re_id+"&terms="+terms+"&nowtime="+(new Date()),
				dataType:"json",
				url:"<%=rootPath%>/linkReportAjaxManager.ajax?action=saveLinkReportOption&type=day",
				success:function(data){
					alert(data.dataStr);
			 		var tab = Ext.getCmp('ext-tab-report');
           			tab.setActiveTab(1);	
...
function deleteItem(id){
	if(window.confirm("您确定要删除吗？")){
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"<%=rootPath%>/linkReportAjaxManager.ajax?action=loadLinkReportList&type=day&id="+id+"&nowtime="+(new Date()),
			success:function(data){
				var modelDiv =document.getElementById("model");
				modelDiv.innerHTML=data.dataStr;
			}
...
function createWin(id){
	return CreateDeviceWindow("<%=rootPath%>/linkReportDay.do?action=linkReportConfig&id="+id);
}
</script>
...
function CreateWindow(url)
{
	...
	function exportReport(exportType){
		...
		window.open('<%=rootPath%>/linkReportDay.do?action=downReport&type=link&reportType=day&exportType='+exportType+'&id='+id+'&terms='+terms+'&ids='+ids+'&startdate='+startdate+'&todate='+todate+'&nowtime='+(new Date()),"_blank","toolbar=no,width=1,height=1,top=2000,left=3000,directories=no,status=no,menubar=no,alwaysLowered=yes");
	}
								...
								<div id='tabs1'>
									<div id="script" class="x-hide-display">
                            			<table border="0" cellpadding="0" cellspacing="0" class="win-content" id="win-content">
											...
											<tr>
                                            	<td width="100%" >
                                               		<jsp:include page="subscribeDay.jsp"></jsp:include>
                                            	</td>
                                    		</tr>
                                    		<tr>
												<td width=100%>
