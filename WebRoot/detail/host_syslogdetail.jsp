...
<b><font color=#ffffff>日志详细信息</font></b>
    </td>   	
...
<INPUT type="button" value="关闭窗口" id=button1 name=button1 onclick="window.close()" class="button">            
  ...
  <tr bgcolor=#cedefa height=25>
    ...
    <td width=20% bgcolor="#F1F1F1">来源(S)</td>
    <td width=30% bgcolor="#F1F1F1"><%=syslog.getProcessname()%></td>
   </tr>
   <tr height=25>
    <td>时间(M)</td>
    <td><%=time1%></td>
    <td>类别(R)</td>
    <td><%=syslog.getProcessname()%></td>
	 </tr>
	 <tr bgcolor="#F1F1F1">
    <td>类型(M)</td>
    <td><%=syslog.getPriorityName()%></td>
    <td>事件(R)</td>
    <td><%=syslog.getEventid()%></td>
  </tr>
 
	 <tr height=25>
    <td>用户(U)</td>
    <td colspan=3><%=syslog.getUsername()%></td>
  </tr>
  
	 <tr bgcolor="#F1F1F1">
    <td>计算机(O)</td>
    <td colspan=3 align=left><%=syslog.getHostname()%></td>
  </tr>
	 <tr height=25>
