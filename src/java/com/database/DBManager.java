...
public class DBManager {
	...
	private DBConnectionManager mg = null;// 数据库管理对象
	...
	public DBManager(String namedConfig) {
		super();
	 ...
	 */
	private void init() {
		mg = new DBConnectionManager(namedConfig);
		try {
 ...
 */
	...
	public boolean excuteBatchSql(Queue<String> list) {
		...
		List<String> batchedSqlList = new ArrayList();
		try{
			...
			if(list.size()>0)
			{
				       String sql="";
						while((sql=list.poll())!=null)
						{
							...
							stmt.addBatch(sql);
							batchedSqlList.add(sql);
						}
						long start = System.currentTimeMillis();
						stmt.executeBatch();
						conn.commit();
						long end = System.currentTimeMillis();
						logger.fatal(mg.getNamedConfig()+"数据库批处理时间="+(end - start) + "ms");
						conn.close();
						stmt.close();
