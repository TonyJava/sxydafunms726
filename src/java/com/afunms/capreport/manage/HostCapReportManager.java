...
public class HostCapReportManager extends BaseManager implements ManagerInterface {
	...
	private String busytimelist() {
		...
		HostNodeDao dao = new HostNodeDao();
		request.setAttribute("list", dao.loadHostByFlag(1));
		return "/capreport/host/busytimelist.jsp";
	}
	...
	private String busytime() {
		...
		if (ids != null && ids.length > 0) {
			for (int i = 0; i < ids.length; i++) {
				...
				if (node == null)
					continue;
				String ip = SysUtil.doip(node.getIpAddress());
				String sql = "select round(avg(convert(h.thevalue,SIGNED)),2) as thevalue,max(convert(h.thevalue,SIGNED)) as maxthevalue,count(h.thevalue)*5 as busytime,DATE_FORMAT(h.collecttime,'%Y-%m-%d %H:%i:%s') as collecttime,h.unit from cpu"+ip+" h where collecttime >='"+starttime+"' and collecttime<='"+totime+"' and thevalue>=10 and right(collecttime,8) >='08:00:00' and right(collecttime,8)<='20:00:00'";
				...
				try {
					while (rs.next()) {
						...
						String maxthevalue = rs.getString(2);
						String busytime = rs.getString(3);
						
						/*List list = new ArrayList();
						list.add(node.getIpAddress());
						list.add(node.getSysName());
						list.add(thevalue);
						list.add(maxthevalue);
						list.add(busytime);*/
						Hashtable busytimehash = new Hashtable();
						if (thevalue != null) {
							
							busytimehash.put("ip", node.getIpAddress());
							busytimehash.put("sysname", node.getIpAddress());
							busytimehash.put("avgcpucon", thevalue);
							busytimehash.put("max", maxthevalue);
							busytimehash.put("busytime", busytime);
							returnList.add(busytimehash);
						}
	...
	private String downloadselfhostreport() {
		...
		if (ConnectUtilizationhash.get("pingMax") != null) {
			ConnectUtilizationmax = (String) ConnectUtilizationhash.get("pingMax");
		}
		maxping.put("pingmax", ConnectUtilizationmax);
	...
	public String execute(String action) {
		...
		if (action.equals("multilist"))
			return multilist();
		if (action.equals("busytimelist"))
			return busytimelist();
		if (action.equals("busytime"))
			return busytime();
		if (action.equals("createpdfbusytimeCpu"))
			return createpdfbusytimeCpu();
		if (action.equals("createdocbusytimeCpu"))
			return createdocbusytimeCpu();
		if (action.equals("downloadcpubusytimereport"))
		   return downloadcpubusytimereport();
...
//			request.setAttribute("filename", fileName);
	 ...
	 * cpu忙时pdf报表
	 */
	public String createpdfbusytimeCpu() {
		String file = "/temp/busytimecpu.pdf";// 保存到项目文件夹下的指定文件夹
		String fileName = ResourceCenter.getInstance().getSysPath() + file;// 获取系统文件夹路径
		try {
			createpdfContextbusytimeCpu(fileName);
		} catch (DocumentException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * cpu忙时pdf报表
	 */
	public void createpdfContextbusytimeCpu(String file) throws DocumentException, IOException {
		// 设置纸张大小
		Document document = new Document(PageSize.A4);
		// 建立一个书写器(Writer)与document对象关联，通过书写器(Writer)可以将文档写入到磁盘中
		PdfWriter.getInstance(document, new FileOutputStream(file));
		...
		// 正文字体风格
		Font contextFont = new Font(bfChinese, 12, Font.NORMAL);
		Paragraph title = new Paragraph("主机服务器忙时CPU利用率报表", titleFont);
		// 设置标题格式对齐方式
		title.setAlignment(Element.ALIGN_CENTER);
		// title.setFont(titleFont);
		document.add(title);
		...
		List cpulist = (List) session.getAttribute("list");// cpu利用率
		...
		Cell cell3 = new Cell(new Phrase("平均值%", contextFont));
		Cell cell4 = new Cell(new Phrase("最大值%", contextFont));
		Cell cell5 = new Cell(new Phrase("忙时总时间(分)", contextFont));
		this.setCellFormat(cell1, true);
		this.setCellFormat(cell11, true);
		this.setCellFormat(cell2, true);
		this.setCellFormat(cell3, true);
		...
		if (cpulist != null && cpulist.size() > 0) {
			for (int i = 0; i < cpulist.size(); i++) {
				...
				Cell cell14 = new Cell(new Phrase(avgcpu.replace("%", "")));
				Cell cell15 = new Cell(new Phrase(cpumax.replace("%", "")));
				Cell cell16 = new Cell(new Phrase(busytime));
	 ...
	 * cpu忙时doc报表
	 */
	public String createdocbusytimeCpu() {
		String file = "/temp/busytimecpu.doc";// 保存到项目文件夹下的指定文件夹
		String fileName = ResourceCenter.getInstance().getSysPath() + file;// 获取系统文件夹路径
		try {
			createDocContextbusytimeCpu(fileName);
		} catch (DocumentException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * cpu忙时doc报表
	 */
	public void createDocContextbusytimeCpu(String file) throws DocumentException, IOException {
		// 设置纸张大小
		Document document = new Document(PageSize.A4);
		// 建立一个书写器(Writer)与document对象关联，通过书写器(Writer)可以将文档写入到磁盘中
		RtfWriter2.getInstance(document, new FileOutputStream(file));
		...
		// 正文字体风格
		Font contextFont = new Font(bfChinese, 10, Font.NORMAL);
		Paragraph title = new Paragraph("主机服务器CPU忙时利用率报表", titleFont);
		// 设置标题格式对齐方式
		title.setAlignment(Element.ALIGN_CENTER);
		// title.setFont(titleFont);
		document.add(title);
	 ...
	 * cpu忙时excel报表
	 */
	private String downloadcpubusytimereport() {
		Date d = new Date();
		String startdate = getParaValue("startdate");
		if (startdate == null) {
			startdate = sdf0.format(d);
		...
		List orderList = new ArrayList();
		orderList = (List) session.getAttribute("list");
		Hashtable reporthash = new Hashtable();
		reporthash.put("cpulist", orderList);
		reporthash.put("starttime", starttime);
		reporthash.put("totime", totime);
		...
		AbstractionReport1 report = new ExcelReport1(new IpResourceReport(), reporthash);
		report.createReport_hostbusytimecpu("/temp/hostcpu_report.xls");
		request.setAttribute("filename", report.getFileName());
		return "/capreport/net/download.jsp";
	}
}
