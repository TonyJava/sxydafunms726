...
//		}
	...
	public void run() {
		...
		while(true){
			...
			SysLogger.info("===syslog==="+packet.getPort());
			SysLogger.info("===syslog==="+packet.getAddress());
			//SysLogger.info("===syslog==="+packet.getSocketAddress());
			SysLogger.info("==============syslog===========================");
				...
				if(host != null){
				   ...
				   // NetSyslogNodeRule noderule = null;
				    ...
				    try{
				    	...
				    	if(ShareData.getSyslogHash().containsKey(host.getId()))
				    	{
				    		nodefacility=(String)ShareData.getSyslogHash().get(host.getId());
				    	}
				    }catch(Exception e){
				    	e.printStackTrace();
				    }finally{
...
//				SysLogger.info("sign---"+sign+"    priority---"+priority);
				if(sign && nodeflist.contains(priority+"")){
					...
					AgentalarmControlutil.GetSyslog_alarmIntosql(syslog);
					//try{
					////	sdao.save(syslog);
					//}catch(Exception e){
					////	e.printStackTrace();
			...
			/**
			 * syslog告警判断
			 * @param message  syslog信息
			 */
			private void createSyslogAlarm(String message) {
				//###########################判断是否要产生syslog告警   根据关键字告警过滤 START######################
				Host host = null;
				try{
					host = (Host)PollingEngine.getInstance().getNodeByIP(ipaddress);
				}catch(Exception e){
				...
				if(containsKeyword){
					...
					checkEvent.setName(host.getId()+":"+nodedto.getSubtype()+":syslog");
