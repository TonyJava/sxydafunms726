...
//	    returnHash.put("flash",flashVector);
	    ...
	    if (!(ShareData.getSharedata().containsKey(node.getIpAddress()))) {
			Hashtable ipAllData = new Hashtable();
			if (ipAllData == null) ipAllData = new Hashtable();
			if (flashVector != null && flashVector.size() > 0) ipAllData.put("light", flashVector);
			ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		} else {
			if (flashVector != null && flashVector.size() > 0) ((Hashtable) ShareData.getSharedata().get(node.getIpAddress())) .put("light", flashVector);
		}
		returnHash.put("light", flashVector);
