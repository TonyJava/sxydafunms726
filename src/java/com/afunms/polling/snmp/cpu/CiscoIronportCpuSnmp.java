...
public class CiscoIronportCpuSnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {	
	    ...
	    try{
			...
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), "gateway", "cisco","cpu");
			for(int i = 0 ; i < list.size() ; i ++){
				AlarmIndicatorsNode alarmIndicatorsnode = (AlarmIndicatorsNode)list.get(i);
				//对CPU值进行告警检测
				CheckEventUtil checkutil = new CheckEventUtil();
				checkutil.updateData(node,collectHash,"gateway","cisco",alarmIndicatorsnode);
				//}
			}
	    }catch(Exception e){
	    	e.printStackTrace();
