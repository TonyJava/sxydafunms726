...
public class LinuxCpuSnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {	
			...
			//-------------------------------------------------------------------------------------------cpu start	
			  ...
			  try{
				...
				if(flag >0){
					...
					temp = intvalue+"";
					SysLogger.info(node.getIpAddress()+" cpu "+allvalue/flag);
				}
	    ...
	    try{
			...
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_HOST, "linux","cpu");
			for(int i = 0 ; i < list.size() ; i ++){
				AlarmIndicatorsNode alarmIndicatorsnode = (AlarmIndicatorsNode)list.get(i);
				//对CPU值进行告警检测
				CheckEventUtil checkutil = new CheckEventUtil();
				checkutil.updateData(node,collectHash,"host","linux",alarmIndicatorsnode);
				//}
			}
	    }catch(Exception e){
	    	e.printStackTrace();
