...
//		    returnHash.put("Cluster", powerVector);
				if (!(ShareData.getSharedata().containsKey(node.getIpAddress()))) {
					...
					if (ipAllData == null) ipAllData = new Hashtable();
					if (powerVector != null && powerVector.size() > 0) ipAllData.put("Cluster", powerVector);
						ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
				} else{
					if (powerVector != null && powerVector.size() > 0) ((Hashtable) ShareData.getSharedata().get(node.getIpAddress())) .put("Cluster", powerVector);
				}	
				returnHash.put("Cluster",powerVector);
		    return returnHash;
		}
