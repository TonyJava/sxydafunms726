...
public class OracleDataCollector {
	...
	public void collect_data(String dbid,Hashtable gatherHash) {
		...
		try{
			...
			OracleEntity oracle = null;
			try {
				oracle = (OracleEntity)partdao.findByID(dbid);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				if (partdao != null)
			...
			try{
				dbmonitorlist = (DBVo)dbdao.findByID(oracle.getDbid()+"");
			}catch(Exception e){
				e.printStackTrace();
			}finally{
				dbdao.close();
			...
			if(oracle.getManaged() == 0)return;
			String serverip = dbmonitorlist.getIpAddress();
			String username = oracle.getUser();
			String passwords = EncryptUtil.decode(oracle.getPassword());
			int port = Integer.parseInt(dbmonitorlist.getPort());
			String dbnames = oracle.getSid();
			List dbList = PollingEngine.getInstance().getDbList();
			DBNode dbnode = (DBNode) PollingEngine.getInstance().getDbByID(oracle.getId());
			...
			if (oracle.getCollectType() == SystemConstant.DBCOLLECTTYPE_SHELL) {
				...
				int flag = 0;
				try {
					oracleIsOK = dbdao.getOracleIsOK(serverip, port, oracle.getSid(), oracle.getUser(), EncryptUtil.decode(oracle.getPassword()));
				}catch(Exception e){
					e.printStackTrace();
				}
				if (!oracleIsOK) {
...
//						this.clearOracleNmsTableData(dbdao,hex + ":" + oracle.getId());
				...
				if(flag == 0){
					...
					SysLogger.info("####开始采集-sid为"+oracle.getId()+"的oracle数据库...");
					Hashtable allOraData =  dbdao.getAllOracleData(serverip, port, oracle.getSid(), oracle.getUser(), passwords,gatherHash);
					SysLogger.info("####采集-sid为"+oracle.getId()+"的oracle数据库耗时+"+(new Date().getTime() - tempDate.getTime()));
					...
					*/
					...
					System.out.println("###################---sid为"+oracle.getId()+"的oracle入库耗时(ms)："+(endDate.getTime()-startDate.getTime()));
					...
					if (tableinfo_v != null && tableinfo_v.size() > 0) {
						...
						Hashtable oraspaces = new Hashtable();
						try{
							oraspaces = oraspaceconfigdao.getByAlarmflag(1);
						}catch(Exception e){
							e.printStackTrace();
						}finally{
					...
					if(info_v != null && info_v.size()>0){
						...
						try{
							...
							dbdao.clear_nmssessiondata(tranfer.formIpToHex(serverip)+":" + oracle.getId());
						}catch(Exception e){
				...
				if(flag == 0){
					...
					dbdao.updateNmsValueByUniquekeyAndTablenameAndKey("nms_orastatus", "serverip", hex + ":" + oracle.getId(), "status", status);
				}
				
				//allFlag == 1
				if(oracledata!=null){
				   ShareData.setAlloracledata(serverip + ":" + oracle.getId(), oracledata);
				   updateData(oracle, oracledata);
				}
			}
		}catch(Exception e){
			e.printStackTrace();
