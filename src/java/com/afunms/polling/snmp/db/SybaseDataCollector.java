...
//					SysLogger.info("#######################################################");
					...
					if(sysbaseIsOK){
						SysLogger.info("#### 可以连接上sybase数据库"+serverip+"，添加指标告警信息   HONGLI####");
...
//							SysLogger.info("##############updatedate--Sybase发送短信############");
							//HONGLI
							createSMS(alarmIndicatorsNode.getType(), alarmIndicatorsNode.getSubtype(), sybaseServer.getAlias() , sybaseServer.getId() + "", alarmIndicatorsNode.getAlarm_info() + " 当前值为：" + value +  alarmIndicatorsNode.getThreshlod_unit() , Integer.valueOf(level) , 1 , sybaseServer.getAlias() , sybaseServer.getBid(),sybaseServer.getAlias() + "(" + sybaseServer.getAlias() + ")");
						}else{
							num_int = num_int + 1;
	...
	private void createEvent(String eventtype,String eventlocation,String bid,String content,int level1,String subtype,String subentity,String ipaddress,String objid){
		//生成事件
		SysLogger.info("##############HONG createEvent开始生成事件############");
