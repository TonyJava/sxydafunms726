...
public class Huawei3comvpn extends BaseVo
{
		...
		try {
			...
			while (flag) {
				...
				if (sb.toString().endsWith(pattern2) ) //如果以pattern1结尾，说明命令执行成功
				{
					...
					if(sb.toString().indexOf("% Login failed!")>0){
	 ...
	 * @return
	 */
	private void readcommand(String pattern,StringBuffer ret,String each) {
		try {
			char lastChar = pattern.charAt(pattern.length() - 1);
			byte[] tempchars = new byte[30];
	...
	public String Backupconffile(String bkpType) {
		...
		if(bkpType.equals("run"))
			result = this.Getcommantvalue1("disp cu");// 获取命令的结果
		else
			result = this.Getcommantvalue1("disp saved-configuration");// 获取命令的结果
		// 对结果进行格式化
...
//		}
	 ...
	 * @param remoteFile   上传后的文件名，如/cfg1.cfg
	 * @param localFile		本地文件名，如C:\\Documents and Settings\\GZM\\cfg1.cfg
	 * @param ftpIp			
	 * @param ftpPort
	 * @param ftpUser
	 * @param ftpPassword
	...
	public String Getcommantvalue1(String[] command) 
	{
		...
		if (loginflg) {
			...
			for (int i = 0; i <command.length; i++) {
				...
				readcommand(DEFAULT_PROMPT,result,each);
				...
				for (int i = 0; i <command.length; i++) {
					...
					if (null != resultTemp && !resultTemp.equals("user or password error")) {
						...
						for (int j = 0; j < st.length; j++) { 
							...
							if(!st[j].startsWith("  ---- More ----"))
							{
								if (st[j].indexOf("% Unrecognized command found at '^' position.")>-1
										||st[j].indexOf("% Ambiguous command found at '^' position.")>-1
										||st[j].indexOf("% Incomplete command found at '^' position.")>-1
										||st[j].indexOf("Error:")>-1) {
									cmdResult.setIp(ip);
	...
	public String isPasswordCorrect()
	{
		...
		if (loginflg && null != suuser && suuser.trim().length() > 0 && null != supassword && supassword.trim().length() > 0) 
		{
			...
			readUntil(this.Loginpsuassword);
			write(this.supassword);
			readUntil1(DEFAULT_PROMPT,"");// 在这里登录成功//bug
			loginflg = true;
		}
		else if (loginflg == false) 
		{
