...
//		System.out.println("begin to send cmd = " + command);
		...
		try
		{
//			System.out.println("begin to open channel ");
			channel = session.openChannel("shell");
//			System.out.println("after you opened the channel ");
		} catch (JSchException e)
		{
			e.printStackTrace();
		}
...
//		System.out.println("after set pty");
...
//					SysLogger.info(line);
					...
					{
//						if(command.indexOf("entstat")>=0)
//						SysLogger.info("&&&&&&&&&& "+i+" ==== "+(results.length - 2));
...
//							SysLogger.info(results[i+1]);
							if(results[i+1].indexOf("Hardware Address:")>=0 || results[i+1].indexOf("load average:")>=0 
									|| results[i+1].indexOf("$hdisk")>=0 || results[i+1].indexOf("BEIST")>=0 || command.equalsIgnoreCase("lsuser ALL")
...
//			 //log(ip , result);
//			 //log(ip , "cmdbegin:cpuconfig");
//			 result = ssh.send("cat /proc/cpuinfo | egrep \"model name|processor|cpu MHz|cache size\"");
//			 //log(ip , result);
//			 log(ip , "cmdbegin:disk");
//			 result = ssh.send("df -k");
//			 log(ip , result);
