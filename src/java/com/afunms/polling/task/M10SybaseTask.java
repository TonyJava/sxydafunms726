...
public class M10SybaseTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行Sybase的10分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的SYBASE所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("10", "m",1,"db","sybase");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
			...
			SysLogger.info("#### M10SYBASETask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
