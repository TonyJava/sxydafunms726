...
public class M10URLTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行URL的10分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的URL所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("10", "m",1,"service","url");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
			...
			SysLogger.info("#### M10URLTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
