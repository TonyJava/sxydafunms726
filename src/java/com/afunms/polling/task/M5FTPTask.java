...
public class M5FTPTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行FTP的5分钟采集任务 ####");
		try{
			NodeGatherIndicatorsDao indicatorsdao = new NodeGatherIndicatorsDao();
	    	List<NodeGatherIndicators> monitorItemList = new ArrayList<NodeGatherIndicators>(); 	
	    	Hashtable<String,Hashtable<String,NodeGatherIndicators>> ftplHash = new Hashtable<String,Hashtable<String,NodeGatherIndicators>>();//存放需要监视的DB2指标  <dbid:Hashtable<name:NodeGatherIndicators>>

	    	try{
	    		//获取被启用的FTP所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("5", "m",1,"service","ftp");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
	    	...
	    	for(int i=0;i<monitorItemList.size();i++){
	    		...
	    		//FTP监视
    			if(ftplHash.containsKey(nodeGatherIndicators.getNodeid())){
    				//若ftpid已经存在,则获取原来的,再把新的添加进去
    				Hashtable gatherHash = (Hashtable)ftplHash.get(nodeGatherIndicators.getNodeid());
    				gatherHash.put(nodeGatherIndicators.getName(), nodeGatherIndicators);
    				ftplHash.put(nodeGatherIndicators.getNodeid(), gatherHash);
    			}else{
    				//若ftpid不存在,则把新的添加进去
        		...
        		//生成FTP采集线程
        		if(ftplHash != null && ftplHash.size()>0){
        			// 生成线程池
            		ThreadPool dbthreadPool = new ThreadPool(ftplHash.size());
        			//存在需要采集的FTP指标
        			for(Enumeration enumeration = ftplHash.keys(); enumeration.hasMoreElements();){
						String ftpid = (String)enumeration.nextElement();
						//SysLogger.info("ftpid=============="+ftpid);
						Ftp node = (Ftp)PollingEngine.getInstance().getFtpByID(Integer.parseInt(ftpid));
						//SysLogger.info("isManaged=============="+node.isManaged());
						//if(!node.isManaged())continue;
						if(node.getMonflag() == 0)continue;
						Hashtable<String,NodeGatherIndicators> gatherHash = (Hashtable<String,NodeGatherIndicators>)ftplHash.get(ftpid);
						dbthreadPool.runTask(createFTPTask(ftpid,gatherHash));
					}
        			// 关闭线程池并等待所有任务完成
            		dbthreadPool.join();
            		dbthreadPool.close();
			...
			SysLogger.info("#### M5FTPTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
    ...
    创建URL采集任务
	 */	
	private static Runnable createFTPTask(final String dbid,final Hashtable gatherHash) {
    return new Runnable() {
        public void run() {
            try {                	
//            	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
            	...
            	SysLogger.info("##############################");
            	SysLogger.info("### 开始采集ID为"+dbid+"的FTP数据 ");
            	SysLogger.info("##############################");
            	ftpcollector.collect_data(dbid, gatherHash);
            }catch(Exception exc){
