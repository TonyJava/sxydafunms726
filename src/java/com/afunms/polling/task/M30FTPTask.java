...
public class M30FTPTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行FTP的30分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的URL所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("30", "m",1,"service","ftp");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
	    	...
	    	for(int i=0;i<monitorItemList.size();i++){
        		...
        		if(urlHash != null && urlHash.size()>0){
        			...
        			for(Enumeration enumeration = urlHash.keys(); enumeration.hasMoreElements();){
						...
						dbthreadPool.runTask(createFTPTask(dbid,gatherHash));
					}
        			// 关闭线程池并等待所有任务完成
            		dbthreadPool.join();
            		dbthreadPool.close();
			...
			SysLogger.info("#### M30FTPTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
