...
public class M30SocketTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行SOCKET的30分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的URL所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("30", "m",1,"service","socket");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
	    	...
	    	for(int i=0;i<monitorItemList.size();i++){
        		...
        		if(urlHash != null && urlHash.size()>0){
        			// 生成线程池
            		ThreadPool threadPool = new ThreadPool(urlHash.size());
        			//存在需要采集的SOCKET指标
        			for(Enumeration enumeration = urlHash.keys(); enumeration.hasMoreElements();){
						...
						threadPool.runTask(createSocketTask(dbid,gatherHash));
					}
        			// 关闭线程池并等待所有任务完成
            		threadPool.join();
            		threadPool.close();
			...
			SysLogger.info("#### M30SOCKETTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
    ...
    创建SOCKET采集任务
	 */	
	private static Runnable createSocketTask(final String id,final Hashtable gatherHash) {
    return new Runnable() {
        public void run() {
            try {                	
//            	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
            	...
            	SysLogger.info("##############################");
            	SysLogger.info("### 开始采集ID为"+id+"的URL数据 ");
            	SysLogger.info("##############################");
            	socketcollector.collect_data(id, gatherHash);
            }catch(Exception exc){
