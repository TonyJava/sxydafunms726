...
public class M30URLTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行URL的30分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的URL所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("30", "m",1,"service","url");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
			...
			SysLogger.info("#### M30URLTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
