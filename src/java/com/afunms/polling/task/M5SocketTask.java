...
public class M5SocketTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行SOCKET的5分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的URL所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("5", "m",1,"service","socket");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
			...
			SysLogger.info("#### M5SOCKETTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
...
//            	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
            	...
            	SysLogger.info("##############################");
            	SysLogger.info("### 开始采集ID为"+id+"的Socket数据 ");
            	SysLogger.info("##############################");
