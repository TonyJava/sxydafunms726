...
public class M5InformixTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行Informix的5分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		...
	    		monitorItemList = indicatorsdao.getByInterval("5", "m",1,"db","informix");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
			...
			SysLogger.info("#### M5INFORMIXTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
