...
public class M30SQLServerTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行SQLServer的30分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的DB2所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("30", "m",1,"db","sqlserver");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
			...
			SysLogger.info("#### M30SQLSERVERTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
