...
public class M30ORACLETask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行ORACLE的30分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的ORACLE所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("30", "m",1,"db","oracle");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
			...
			SysLogger.info("#### M30ORACLETask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
