...
public class M1URLTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行URL的1分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的URL所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("1", "m",1,"service","url");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
	    	...
	    	for(int i=0;i<monitorItemList.size();i++){
        		...
        		if(urlHash != null && urlHash.size()>0){
        			...
        			for(Enumeration enumeration = urlHash.keys(); enumeration.hasMoreElements();){
						...
						Node node = (Node)PollingEngine.getInstance().getWebByID(Integer.parseInt(dbid));
						Hashtable<String,NodeGatherIndicators> gatherHash = (Hashtable<String,NodeGatherIndicators>)urlHash.get(dbid);
						dbthreadPool.runTask(createURLTask(dbid,gatherHash));
					}
        			// 关闭线程池并等待所有任务完成
            		dbthreadPool.join();
            		dbthreadPool.close();
			...
			SysLogger.info("#### M5URLTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
    ...
    创建URL采集任务
	 */	
	private static Runnable createURLTask(final String dbid,final Hashtable gatherHash) {
    return new Runnable() {
        public void run() {
            try {                	
...
//            	SysLogger.info("##############################");
//            	SysLogger.info("### 开始采集ID为"+dbid+"的URL数据 ");
//            	SysLogger.info("##############################");
            	urlcollector.collect_data(dbid, gatherHash);
            }catch(Exception exc){
