...
public class M5SybaseTask extends MonitorTask {
	
	public void run() {
		SysLogger.info("#### 开始执行Sybase的5分钟采集任务 ####");
		try{
			...
			try{	
				...
				try {
					...
					db2typevo = (DBTypeVo) typedao.findByDbtype("sybase");
				} catch (Exception e) {
					e.printStackTrace();
				} finally {
					if (typedao != null)
			...
			SysLogger.info("#### M5SYBASETask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
