...
public class M5MailTask extends MonitorTask {
	
	public void run() {
		
//		SysLogger.info("#### 开始 AllTypeMap 刷新 ####");
		
		HashMap<String,List> hashMap=new HashMap<String,List>();
		hashMap=PollingEngine.getInstance().getAllTypeMap(); 
		
		PollingEngine.getInstance().mapRefresh(); 
		
		
		
//		HashMap<String,List> hashMap1=new HashMap<String,List>();
//		hashMap1=PollingEngine.getInstance().getAllTypeMap();  
//		System.out.println("---刷新 后---$$$$$$$$$$$$$$$%%%%%%%%%%"); 
//		System.out.println(hashMap1);   
//		SysLogger.info("#### 结束 AllTypeMap 刷新 ####");
		SystemConfigDao systemConfigDao = new SystemConfigDao();
		String collectwebflag = "0";
		try {
			collectwebflag = systemConfigDao.getSystemCollectByVariablename("collectwebflag");
			PollingEngine.getInstance().setCollectwebflag(collectwebflag);
		} catch (Exception e) {
			e.printStackTrace();
		} finally{
			systemConfigDao.close();
		}
		SysLogger.info("#####刷新系统运行模式   当前运行模式为："+String.valueOf((collectwebflag.equals("1")?"分离模式":"集成模式"))+"#######");
		
		SysLogger.info("#### 开始执行MAIL的5分钟采集任务 ####");
		try{
	    	...
	    	try{
	    		//获取被启用的URL所有被监视指标
	    		monitorItemList = indicatorsdao.getByInterval("5", "m",1,"service","mail");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
	    	...
	    	for(int i=0;i<monitorItemList.size();i++){
        		...
        		if(urlHash != null && urlHash.size()>0){
        			...
        			for(Enumeration enumeration = urlHash.keys(); enumeration.hasMoreElements();){
						...
						Node node = (Node)PollingEngine.getInstance().getMailByID(Integer.parseInt(dbid));
						Hashtable<String,NodeGatherIndicators> gatherHash = (Hashtable<String,NodeGatherIndicators>)urlHash.get(dbid);
						dbthreadPool.runTask(createMAILTask(dbid,gatherHash));
					}
        			// 关闭线程池并等待所有任务完成
            		dbthreadPool.join();
            		dbthreadPool.close();
			...
			SysLogger.info("#### M5MAILTask Thread Count : "+Thread.activeCount()+" ####");
		}
	}
    ...
    创建URL采集任务
	 */	
	private static Runnable createMAILTask(final String dbid,final Hashtable gatherHash) {
    return new Runnable() {
        public void run() {
            try {                	
//            	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
            	...
            	SysLogger.info("##############################");
            	SysLogger.info("### 开始采集ID为"+dbid+"的MAIL数据 ");
            	SysLogger.info("##############################");
            	mailcollector.collect_data(dbid, gatherHash);
            }catch(Exception exc){
