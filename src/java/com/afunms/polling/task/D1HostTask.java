...
public class D1HostTask extends MonitorTask {
...
//        		try{
//        		allsystemdataHash = xmlutil.parserXml(ResourceCenter.getInstance().getSysPath()+ "linuxserver/host_systemgroup.xml");
    ...
    return new Runnable() {
        public void run() {
            try {                	
            	...
            	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
            	Host host = (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(nodeGatherIndicatorsNode.getNodeid()));
            	if (host==null) return;
