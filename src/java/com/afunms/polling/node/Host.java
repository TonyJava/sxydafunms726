...
//			            				if(this.alarmlevel < flag)this.alarmlevel=flag;
			        			...
			        			if("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())){
			        				int flag = 0;
			        				flag = checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"memory");
			            			if(flag>0){
			            				//有告警产生
			            				alarmMsg.append(alarmIndicatorsNode.getAlarm_info()+"<br>");
//			            				alarm = true;
...
//			                				if(this.alarmlevel < flag)this.alarmlevel=flag;
			    				...
			    				if("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())){
			    					...
			    					int flag = 0;
			    					flag = checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"AllInBandwidthUtilHdx");
			        				if(flag>0){
			            				//有告警产生
			            				...
			            				for(int si=0;si<diskVector.size();si++){
			            					...
			            					if(diskdata.getEntity().equalsIgnoreCase("Utilization")){
			            						...
			            						if(getOstype() ==4 || getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")){
			            							flag = checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"diskperc:diskperc:"+diskdata.getSubentity().substring(0, 3));
			            						}else{
			            							flag = checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"diskperc:diskperc:"+diskdata.getSubentity());
			            						}
			            	        			if(flag>0){
			            	        				//有告警产生
...
//			            	        			}
			        		...
			        		}else if(alarmIndicatorsNode.getName().equals("physicalmemory")){
			        			//物理内存信息
			        			if("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())){
			            			if (memoryVector != null && memoryVector.size()>0){
...
//			            	        				if(this.alarmlevel < flag)this.alarmlevel=flag;
			        		...
			        		}else if(alarmIndicatorsNode.getName().equals("virtualmemory")){
			        			//物理内存信息
			        			if("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())){
			            			if (memoryVector != null && memoryVector.size()>0){
			            				for(int si=0;si<memoryVector.size();si++){
...
//			            	        				if(this.alarmlevel < flag)this.alarmlevel=flag;
			        		...
			        		} else if(alarmIndicatorsNode.getName().equals("swapmemory")){
								//交换内存信息
			        			if("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())){
			        				if (memoryVector != null && memoryVector.size()>0){
					    				for(int si=0;si<memoryVector.size();si++){
