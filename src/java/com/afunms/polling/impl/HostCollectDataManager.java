...
public class HostCollectDataManager implements I_HostCollectData {
								...
								try {
									...
									if (flag) {
										...
										try {
											...
											if (ftpConfig != null) {
												FtpUtil ftputil = new FtpUtil(ftpConfig.getIp(), 21, ftpConfig.getUsername(), ftpConfig.getPassword(), "", ResourceCenter.getInstance().getSysPath()
														+ "/linuxserver/", "");
												uploadsuccess = ftputil.uploadFile(ftpConfig.getIp(), ftpConfig.getUsername(), ftpConfig.getPassword(), ResourceCenter.getInstance().getSysPath()
														+ "/linuxserver/" + type + "_arp" + host.getIpAddress() + ".xml");
											}
		...
		try {
			...
			if (!starttime.equals("") && !endtime.equals("")) {
				...
				if (category.equals("Ping") || category.equals("CPU") || category.equals("Memory") || category.equals("dominoCpu") || category.equals("domplatmem") || category.equals("domservmem")
						|| category.equals("Process") || category.equals("MqPing") || category.equals("DomPing")) {
					sid = SysUtil.doip(ip);
				}
				...
				if (category.equals("Ping")) {
					//
					if("phoenix".equalsIgnoreCase(DB_TYPE)){
						sb.append("select h.thevalue,h.collecttime,h.unit from ping"+sid+" h where ");
					}
				else if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
					...
					if("phoenix".equalsIgnoreCase(DB_TYPE)){
						sb.append("select h.thevalue,to_char(h.collecttime,'yyyy-MM-dd HH:mm:ss') as collecttime,h.unit from cpu"+sid+" h where ");
					}
				else if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				...
				if("phoenix".equalsIgnoreCase(DB_TYPE)) {
					...
					sb.append(starttime);
					sb.append("','yyyy-MM-dd HH:mm:ss','local') and h.collecttime <= to_date('");
					sb.append(endtime);
					sb.append("','yyyy-MM-dd HH:mm:ss','local')");
	...
	private  final Log logger = LogFactory.getLog(HostCollectDataManager.class);
	public Hashtable[] getMemory(String ip, String category, String starttime, String endtime)   {
		Connection con = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		DBManager dbmanager = new DBManager(DB_TYPE);
		...
		try {
			...
			String sql1 = "";
			if("phoenix".equalsIgnoreCase(DB_TYPE)){
				sql1="select max(to_number(h.thevalue)) as thevalue,h.subentity,avg(to_number(h.thevalue)) as avgvalue from memory" + allipstr + " h " +
						"where  h.category='Memory' and h.collecttime >= to_date('" + starttime + "','yyyy-MM-dd HH:mm:ss','local') and h.collecttime <= to_date('"+endtime+"','yyyy-MM-dd HH:mm:ss','local') and h.thevalue != 'NaN' " +
						"group by h.subentity order by h.subentity";
			}
		else if(SystemConstant.DBType.equals("mysql")){
...
//						"where  h.category='Memory' and h.collecttime >= '2012-04-06 00:00:00' " +
//						"and h.collecttime <= '2012-04-06 23:59:59' and h.thevalue != 'NaN' " +
//						"group by h.subentity order by h.subentity";
				
				sql1="select max(convert(h.thevalue,SIGNED)) as thevalue,h.subentity,avg(convert(h.thevalue,SIGNED)) as avgvalue from memory" + allipstr + " h " +
				"where  h.category='Memory' and h.collecttime >= '" + starttime + "' and h.collecttime <= '"+endtime+"' and h.thevalue != 'NaN' " +
				"group by h.subentity order by h.subentity";
			...
			System.out.println("HostCollectDataManager.class  8230//////////////////////////"+sql1);
			...
			if (list1.size() != 0) {
				...
				StringBuffer sb = new StringBuffer();
				if("phoenix".equalsIgnoreCase(DB_TYPE)){
					sb.append("select h.thevalue,to_char(h.collecttime,'yyyy-MM-dd HH:mm:ss') as collecttime,h.unit,h.subentity from memory" + allipstr + " h where ");
				...
				if("phoenix".equalsIgnoreCase(DB_TYPE)){
					sb.append("' and h.collecttime >= to_date('"+starttime+"','yyyy-MM-dd HH:mm:ss','local') and h.collecttime <= to_date('"+endtime+"','yyyy-MM-dd HH:mm:ss') order by h.collecttime");
				...
				// System.out.println(sql);
				dbmanager.close();
				dbmanager = new DBManager(DB_TYPE);
				rs = dbmanager.executeQuery(sql);
				while (rs.next()) {
					Vector v = new Vector();
					String thevalue = rs.getString("thevalue");
	...
	public Hashtable getAllutilhdx(String ip, String subentity, String starttime, String endtime, String need, String time) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
				sb.append("select h.thevalue,DATE_FORMAT(h.collecttime,'%Y-%m-%d %H:%i:%s') as collecttime,h.unit from allutilhdx" + time + allipstr + " h where ");
				sb.append("h.subentity='");
				sb.append(subentity);
				sb.append("' and h.collecttime >= '");
				sb.append(starttime);
				...
				sb.append("select h.thevalue,TO_CHAR(h.collecttime, 'YYYY-MM-DD HH24:MI:SS') as collecttime,h.unit from allutilhdx" + time + allipstr + " h where ");
				sb.append(" h.subentity='");
				sb.append(subentity);
				sb.append("' and h.collecttime >= to_date('");
				sb.append(starttime);
...
//			sb.append("' order by h.collecttime");
			...
			try {
				while (rs.next()) {
					...
					String thevalue = rs.getString("thevalue");
					String collecttime = rs.getString("collecttime");
//					System.out.println(thevalue+"----------------------------"+collecttime);
					v.add(0, emitStr(thevalue));
					v.add(1, collecttime);
					v.add(2, rs.getString("unit"));
					avgput = avgput + getfloat(thevalue);
...
//		}
	...
	public Hashtable getCurByCategory(String ip, String category,
			String subentity, String startTime, String endTime)
			throws Exception {
 		Hashtable hash = new Hashtable();
 		Connection con = null;
 		PreparedStatement stmt = null;
 		...
 		try {
 			if (!starttime.equals("") && !endtime.equals("")) {
 				...
 				if (category.equals("Ping")) {
 					...
 					sb.append(" select h.thevalue,DATE_FORMAT(h.collecttime,'%Y-%m-%d %H:%i:%s') as collecttime,h.unit from db2ping"
 									+ allipstr + " h where ");
 				} else if (category.equals("SYSPing")) {
 					sb
 					...
 					sb.append(" select h.thevalue,DATE_FORMAT(h.collecttime,'%Y-%m-%d %H:%i:%s') as collecttime,h.unit from cpuhour"
 							+ allipstr + " h where ");
 				} else if (category.equals("Memory")) {
 					sb
