...
//				allipstr = ip1 + ip2 + ip3 + ip4;
				...
				if (typevo.getDbtype().equalsIgnoreCase("sqlserver")) {
					...
					ctable.deleteTable(conn, "informixlog", allipstr, "informixlog");//日志信息
				}
...
//			allipstr = ip1 + ip2 + ip3 + ip4;
			...
			if (typevo.getDbtype().equalsIgnoreCase("sqlserver")) {
				try {
					...
					ctable.deleteTable(conn, "sqlpinghour", allipstr, "sqlpinghour");//Ping
					ctable.deleteTable(conn, "sqlpingday", allipstr, "sqlpingday");//Ping 
					ctable.dropDbconfigInfo(conn, "system_sqldbconf", ip);//告警阀值
				} catch (Exception ex) {
				...
				try {
					...
					ctable.deleteTable(conn, "orapinghour", allipstr, "orapinghour");//Ping
					ctable.deleteTable(conn, "orapingday", allipstr, "orapingday");//Ping   
					ctable.dropDbconfigInfo(conn, "system_oraspaceconf", ip);//告警阀值 
				...
				try {
					...
					ctable.deleteTable(conn, "db2pinghour", allipstr, "db2pinghour");//Ping
					ctable.deleteTable(conn, "db2pingday", allipstr, "db2pingday");//Ping     
					ctable.dropDbconfigInfo(conn, "system_db2spaceconf", ip);//告警阀值 
				} catch (Exception ex) {
				...
				try {
					...
					ctable.deleteTable(conn, "syspinghour", allipstr, "syspinghour");//Ping
					ctable.deleteTable(conn, "syspingday", allipstr, "syspingday");//Ping  
					ctable.dropDbconfigInfo(conn, "system_sybspaceconf", ip);//告警阀值
				} catch (Exception ex) {
				...
				try {
					...
					ctable.deleteTable(conn, "mypinghour", allipstr, "mypinghour");//Ping
					ctable.deleteTable(conn, "mypingday", allipstr, "mypingday");//Ping 
					ctable.dropDbconfigInfo(conn, "system_mysqlspaceconf", ip);//告警阀值
				} catch (Exception ex) {
				...
				try {
					...
					ctable.deleteTable(conn, "informixpinghour", allipstr, "informixpinghour");//Ping
					ctable.deleteTable(conn, "informixpingday", allipstr, "informixpingday");//Ping 
					ctable.dropDbconfigInfo(conn, "system_infomixspaceconf", ip);//告警阀值
					ctable.deleteTable(conn, "informixlog", allipstr, "informixlog");//日志信息
				} catch (Exception ex) {
...
//			System.out.println(sBuffer.toString());
	...
	/**
	 * 将mysql的采集信息都放到nms_mysqlinfo表中
	 * 通过 typename 来区分是属于哪一类的信息
	 * typename:
	 * 1、status
	 * 2、dbnames
	 * 3、global_status
...
//						}
	...
	public boolean getOracleIsOK(String ip, int port, String db, String username, String password) {
		...
		String sql = "select a.tablespace_name \"Tablespace\", " +
				...
				"order by a.tablespace_name";
		 */
		String sql = "select * from sys.dual";
		OracleJdbcUtil util = null;
		ResultSet rs = null;
		try {
			util = new OracleJdbcUtil(dburl, username, password);
	...
	public Hashtable collectSQLServerMonitItemsDetail(String ip, String dbname, String username, String password,Hashtable gatherHash) 
	{
		...
		/* 取得系统表master..sysperfinfo的信息 
		 * 第一列为object_name: 性能对象名，如 SQL Server：锁管理器或 SQL Server：缓冲区管理器
		 * 第二列为counter_name: 对象内的性能计数器名称，如页请求或请求的锁
		 * 第三列为instance_name: 计数器的命名实例。例如，有为各类型的锁（如表锁、页锁、键锁等）维护的计数器。实例名在相似的计数器之间是有区别的
		 * 第四列为value: 结合cntr_value，cntr_type属性，经过分析和计算得到的性能值
		 * */
		...
		pages.put("planCacheHitRatio", planCacheHitRatio + ""); //*$ plan cache击中率 
		pages.put("cursorManagerByTypeHitRatio", cursorManagerByTypeHitRatio + ""); //*$ Cursor Manager by Type击中率 
		pages.put("catalogMetadataHitRatio", catalogMetadataHitRatio + ""); //*$ Catalog Metadata击中率
		...
		pages.put("totalPageLookupsRate", totalPageLookupsRate + "");//*  查找页数/分
		...
		mems.put("grantedWorkspaceMem", grantedWorkspaceMem + ""); //*  产生的工作空间内存
		...
		sqls.put("sqlRecompilationRate", sqlRecompilationRate + ""); //*  SQL重编辑/分
		...
		sqls.put("failedAutoParamsRate", failedAutoParamsRate + "");//*  自动参数化失败次数/分
...
//			SysLogger.info("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
//			SysLogger.info("&&&&&&&&&&&开始连接INFORMIX 数据库 IP:"+ip+"&&&&&&&&&&");
//			SysLogger.info("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
			util.jdbc();
			rs = util.stmt.executeQuery(sql);
			if (rs.next()) {
...
//			rs.close();
			...
			while (rs.next()) {
				...
				/*informixsession.put("logrecs", rs.getInt("logrecs").trim());//已写入逻辑日志的记录数
				informixsession.put("longtxs", rs.getInt("longtxs").trim());//长事物数*/
				informixsession.put("bufreads", rs.getInt("bufreads"));//缓冲区读取数
				informixsession.put("bufwrites", rs.getInt("bufwrites"));//缓冲区写入数
				//informixsession.put("seqscans", rs.getInt("seqscans").trim());//顺序扫描数
				informixsession.put("pagreads", rs.getInt("pagreads"));//页读取数
				informixsession.put("pagwrites", rs.getInt("pagwrites"));//页写入数
				/*informixsession.put("total_sorts", rs.getInt("total_sorts").trim());//排序总数
				informixsession.put("dsksorts", rs.getInt("dsksorts").trim());//不适合内存的排序数
				informixsession.put("max_sortdiskspace", rs.getInt("max_sortdiskspace").trim());//排序所使用的最大空间*/
			...
			/*rs = util.stmt
					.executeQuery("select sum(size) from syslogs"); //逻辑文件所占空间
	...
	try {
		...
		util.jdbc();
		ResultSet rs = null;
		//System.out.println("-===---------------------1-------------------------------------=="+gatherHash.toString());
	//判断是否获取系统信息
	if(gatherHash.containsKey("sysvalue")){
		try{
		rs = util.stmt.executeQuery(sql);
		...
		while (rs.next()) {
			...
			//System.out.println("===result1="+databaseVal.toString());
	...
	if(gatherHash.containsKey("log")){
		...
		while (rs.next()) {
			...
			//System.out.println("===result2="+informixlog.toString());
	...
	if(gatherHash.containsKey("space")){//sysmaster
		...
		while (rs.next()) {
			...
			//if (names.get(name) == null) {
				...
				//System.out.println("===result3="+informixspaces.toString());
				spaceList.add(informixspaces);
				//names.put(name, 1);
			/*} else {
				for (int i = 0; i < spaceList.size(); i++) {
	...
	if(gatherHash.containsKey("config")){
		...
		while (rs.next()) {
			...
			//System.out.println("===result4="+informixconfig.toString());
			configList.add(informixconfig);
		}
		}catch(Exception e){
		}finally{
	...
	if(gatherHash.containsKey("session")){
		...
		while (rs.next()) {
			...
			//System.out.println("==result 5="+informixsession);
			sessionList.add(informixsession);
		}
		}catch(Exception e){
		}finally{
...
//			SysLogger.info("pageswritten ==== "+informixio.get("pageswritten"));
		...
		////System.out.println("==result 7="+informixio);
			iolist.add(informixio);
		}
		}catch(Exception e){
		}finally{
...
//		}
//	//}
		//System.out.println("====================offset==============================#"+offset.trim()+"#===");
		//System.out.println("====================offset==============================#"+offset.trim().length()+"#===");
		//*******分段取sysonlinelog中的数据-START*******
		int count = -1;
		try {
			//System.out.println("====================offset=============1=================#"+(offset.trim().length()>0)+"#===");
...
//				{
//					ee.printStackTrace();
//					//System.out.println("===========================count===============44=============="+count);
//				}
...
//		//遍历info_v
//		if(info_vHash != null && !info_vHash.isEmpty()){
//			Iterator ipidIterator = info_vHash.keySet().iterator();
//			while (ipidIterator.hasNext()) {
//				String serverip = String.valueOf(ipidIterator.next());
//				Vector info_v = (Vector)info_vHash.get(serverip);
//				String deletesql = "delete from nms_orarollback where serverip='"+serverip+"'";
//				dbManager.addBatch(deletesql);
//				
//				
