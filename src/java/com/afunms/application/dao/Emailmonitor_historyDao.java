...
public class Emailmonitor_historyDao extends BaseDao implements DaoInterface
{
	...
	*/
	...
	public Vector getByMailid(Integer mailid,String starttime,String totime,Integer isconnected) throws Exception{
    	...
    	Vector returnVal = new Vector();
    	try{
			String sql = "select is_canconnected," +
...
//			SysLogger.info(sql);
	...
	public String[] getAvailability(Integer mail_id,String starttime,String totime,String type)throws Exception{
		...
		try {
			...
			parm=parm+totime;
			parm=parm+"'";
			String sql = "select sum(aa."+type+") as stype ,COUNT(aa.email_id) as countid from nms_email_history aa where aa.email_id="+mail_id+" and "+parm;
			rs = conn.executeQuery(sql);
			while(rs.next()){
				value[0] = rs.getInt("stype")+"";
				value[1] = rs.getInt("countid")+"";
 ...
 //yangjun add
   public Hashtable getPingData(Integer email_id,String starttime,String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			String sql = "select a.is_canconnected,a.reason,a.mon_time from nms_email_history a where " +
