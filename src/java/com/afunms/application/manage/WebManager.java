    ...
    private String showPingReport() {
		...
		try {
			...
			if (ConnectUtilizationhash.get("curPing") != null) {
				...
				.get("curPing");    //取当前连通率可直接从 nms_ftp_history表获取,没必要再从nms_ftp_realtime表获取
			}
...
//			}
			if (ConnectUtilizationhash.get("curPing") != null) {
				curPing = (String) ConnectUtilizationhash
				.get("curPing");    //取当前连通率可直接从 nms_web_history表获取,没必要再从nms_web_realtime表获取
			}
			...
			pollMonitorManager.p_draw_line(
					(List)ConnectUtilizationhash.get("pingList"), "连通率",
					newip + "pingConnect", 740, 150);
			pollMonitorManager.p_draw_line(
					(List)ConnectUtilizationhash.get("delayList"), "时延",
					newip + "delayConnect", 740, 150);
			pollMonitorManager.p_draw_line(
					(List)ConnectUtilizationhash.get("pageList"), "页面数据包大小",
					newip + "pageConnect", 740, 150);
			pollMonitorManager.p_draw_line(
					(List)ConnectUtilizationhash.get("changeList"), "页面修改率",
