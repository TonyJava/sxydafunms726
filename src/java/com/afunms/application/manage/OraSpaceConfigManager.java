...
public class OraSpaceConfigManager extends BaseManager implements ManagerInterface {
		...
		OraclePartsDao oracledao = null;	
			if (oraList != null && oraList.size() > 0) {
				for (int i = 0; i < oraList.size(); i++) {
					...
					List oracles = oracledao.getOraclesByDbAndBid(dbmonitorlist.getId(),rbids);
					...
					for (int i = 0; i < dblist.size(); i++) {
						...
						try{
							...
							for (OracleEntity ora : oracles) {
								if (oraHash.containsKey(ip1 + ":" + ora.getId()))
									ips.add(dbvo.getIpAddress() + ":" + ora.getSid());
							}
						}catch(Exception e){
							e.printStackTrace();
						}finally{
	...
	private String search() {
		...
		OracleEntity en = null;
		try {
			en = (OracleEntity) oracledao.getOracleBySId(vo.getId(), ipaddr[1]);
		} finally {
			oracledao.close();
		}
		try {
			...
			try {
				...
				if (oraHash != null && oraHash.size() > 0 && dblist != null && dblist.size() > 0) {
					for (int i = 0; i < dblist.size(); i++) {
						DBVo dbvo = (DBVo) dblist.get(i);
						oracleDao = new OraclePartsDao();
						List<OracleEntity> oracles = oracleDao.findOracleParts(dbvo.getId());
			...
			if (ipaddress != null && ipaddress.trim().length() > 0) {
				...
				try {
					...
					list = configdao.getByIp(ip8 + ":" + en.getId());
				} catch (Exception e) {
					e.printStackTrace();
				} finally {
					configdao.close();
		...
		request.setAttribute("iplist", ips);
		request.setAttribute("ipaddress", hex + ":" + en.getId());// ipaddress
		System.out.println(hex + ":" + en.getId() + "!!!!!!!!!!!!!!!!!!!!!");
	...
	private String ready_edit() {
		...
		try{
			...
			OracleEntity entity=(OracleEntity)oracledao.getOracleById(Integer.parseInt(ips[1]));
