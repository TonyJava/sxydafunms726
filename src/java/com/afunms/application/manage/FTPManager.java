	...
	private String add() {
		...
		try {
			...
			result = timeShareConfigUtil.saveTimeShareConfigList(request,
					String.valueOf(ftpConfig.getId()), timeShareConfigUtil
							.getObjectType("2"));
			...
			timeGratherConfigUtil.saveTimeGratherConfigList(request, String
					.valueOf(ftpConfig.getId()), timeGratherConfigUtil
					.getObjectType("2"));
			/* snow add end */
			...
			try {
				...
				nodeGatherIndicatorsUtil.addGatherIndicatorsForNode(ftpConfig
						.getId()
						+ "", "service", "ftp", "1");
			...
			try {
				...
				alarmIndicatorsUtil.saveAlarmInicatorsThresholdForNode(String
						.valueOf(ftpConfig.getId()), "service", "ftp");
			} catch (RuntimeException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			if (ids != null && ids.length > 0) {
				...
				for (int i = 0; i < ids.length; i++) {
					...
					timeShareConfigUtil.deleteTimeShareConfig(ids[i],
							timeShareConfigUtil.getObjectType("2"));
					...
					if (weslist != null && weslist.size() > 0) {
						for (int j = 0; j < weslist.size(); j++) {
							...
							if (wesvo != null) {
								...
								lineDao.deleteByidXml("ftp" + id, wesvo
										.getXmlfile());
								NodeDependDao nodeDependDao = new NodeDependDao();
								if (nodeDependDao.isNodeExist("ftp" + id, wesvo
										.getXmlfile())) {
									nodeDependDao.deleteByIdXml("ftp" + id,
											wesvo.getXmlfile());
								} else {
									nodeDependDao.close();
								}
	...
	private String readyEdit() {
		...
		try {
			...
			List timeShareConfigList = timeShareConfigUtil
					.getTimeShareConfigList(String.valueOf(ftpConfig.getId()),
							timeShareConfigUtil.getObjectType("2"));
			...
			List<TimeGratherConfig> timeGratherConfigList = tg
					.getTimeGratherConfig(String.valueOf(ftpConfig.getId()), tg
							.getObjectType("2"));
