...
public class NetworkDeviceAjaxManager extends AjaxBaseManager implements AjaxManagerInterface {
	...
	private void saveDkdbTemplate() {
		...
		String time = sdFormat.format(cc);
		StringBuffer sql = new StringBuffer();
		sql.append("insert into nms_dkdbreport(name,collecttime,begintime,endtime) values('");
		sql.append(reportname);
		sql.append("','");
		sql.append(time);
		sql.append("','");
		...
		/*保存端口对比报表中配置的端口号列表*/
		int key = GeneratorKey.getInstance().getNextKey("nms_dkdbreport");
		String ids = request.getParameter("ids");
		try {
	...
	private void loadDkdbTemplet() {
		...
		StringBuffer html = new StringBuffer();
		String id = this.getParaValue("id");
		String sql = "select n.id,n.name,s.EMAIL,s.REPORT_SENDDATE from nms_dkdbreport n,sys_dkdb_ssresources s where n.id=s.SUBSCRIBE_ID";
		List<SubscribeResources> list = new ArrayList<SubscribeResources>();
...
//				sr.setEmailcontent(rs.getString("ids"));// 为方便暂用字段充当性能指标
	...
	private void deleteDkdbTemplet() {
		DBManager dbManager = new DBManager();
		String id = this.getParaValue("id");
		String sql1 = "delete from nms_dkdbreport where id=" + id;
    ...
    private void exeCmd(){
		...
		if(vo.getDeviceRender().equals("h3c")){//h3c
			...
			if(telnet.login()){
				result = telnet.getCommantValue(vo.getSupassword(), commStr);
...
//					end_vector = hostlastmanager.getInterface(endnode.getIpAddress(),netInterfaceItem,"index","","");
    ...
    public void createLinkRoadPic(){
		...
		if(downLinksHashtable != null && !downLinksHashtable.isEmpty()){
			...
			while(linkIdIterator.hasNext()){
				...
				System.out.println(linkHash.get(linkid).getLinkName()+"下行流速");
				linkUtilhdxdivData.append("'> ");
				//遍历该link的上行流速集合集合
				for(int j=0; j<linkidData.size(); j++){
					linkUtilhdxdivData.append("<value xid='");
    ...
    public void verifyLogin() {
		...
		String[] commStr={""};
		result="验证失败！";
    	if (type.equals("h3c")) {// h3c
			Huawei3comvpn tvpn = new Huawei3comvpn();
			tvpn.setSuuser(suuser);// su
			tvpn.setSupassword(supassword);// su密码
			tvpn.setUser(username);// 用户
			tvpn.setPassword(pwd);// 密码
			tvpn.setIp(ipAddress);// ip地址
			tvpn.setDEFAULT_PROMPT(promtp);// 结束标记符号
			tvpn.setPort(port);
			result = tvpn.BackupConfFile(commStr);
		} else if (type.equals("cisco")) {// cisco
			CiscoTelnet telnet = new CiscoTelnet(ipAddress, username,pwd);
			if (telnet.login()) {
				result = telnet.getFileCfg(supassword, commStr);
			} else {
				result="user or password error";
			}

		}
    	if(result.indexOf("% Login invalid")>-1||result.indexOf("% Login failed!")>-1||result.indexOf("user or password error")>-1){
    		result="验证失败";
    	}else {
    		result="验证成功！";
		}
...
//			cpuValueDouble = new Double(new NetService(host.getId()+"", nodedto.getType(), nodedto.getSubtype()).getCurrCpuAvgInfo());
...
//		JFreeChart chart = ChartFactory.createPieChart(title, piedata, false, true, true);
//		//			设定图片标题
