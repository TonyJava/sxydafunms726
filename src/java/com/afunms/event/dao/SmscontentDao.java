...
public class SmscontentDao {
	...
	public boolean sendDatabaseSmscontent(Smscontent smscontent) throws Exception {
		...
		try {
			...
			if(smsObjid.indexOf("oracle")!=-1){
				...
				dbmonitorlist = (DBVo) dbdao.findByID(smscontent.getObjid());
			}
							...
							for (int j = 0; j < ids.length; j++) {
								...
								if (op.getEmail() != null && op.getEmail().trim().length() > 0) {
									...
									if (flag == false) {
										...
										// 写事件*****************************************
										try {
											createEvent("poll", entityId, dbmonitorlist.getAlias() + "("
			...
			// 开始写事件
			try {
				createEvent("poll", entityId, dbmonitorlist.getAlias() + "(" + dbmonitorlist.getIpAddress() + ")",
	...
	public boolean sendSms(String time, Host host, Portconfig portconfig, String value) {
		...
		String smscontent = time + " Trap告警   " + host.getAlias() + value;
		// 开始发送短信
		// 写事件
...
//		}
		...
		eventlist.setOid(0);
		eventlist.setSubtype("network");
		eventlist.setSubentity("link");
		...
		String[] address = null;
	    String userIds = host.getSendemail();
		...
		Address[] add = mailManager.getAddress(address);
		mailManager.SendMail(add, smscontent);
		return true;
	}
