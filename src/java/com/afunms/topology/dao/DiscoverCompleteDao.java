...
public class DiscoverCompleteDao extends BaseDao
{
   ...
   public void addInterfaceData(List hostList)
   {
      ...
      for(int i=0;i<hostList.size();i++)
      {
    	 ...
    	 {
    		 conn = new DBManager();
    		 com.afunms.discovery.IfEntity ifEntity = (com.afunms.discovery.IfEntity)ifList.get(j);
...
//					ctable.deleteTable(conn,"temperday",allipstr,"temperday");
				...
				if(DiscoverEngine.getInstance().getDiscovermodel() == 1){
					...
					ctable.deleteTable(conn,"diskincrehour",allipstr,"diskincrehour");//磁盘增长率小时
					ctable.deleteTable(conn,"diskincreday",allipstr,"diskincreday");//磁盘增长率天
...
//			}
	     ...
	     try{
		    ...
		    String strSql = "";
		    strSql = "insert into nms_netsyslogrule_node(id,nodeid,facility)values(0,'"+node.getId()+"','"+strFacility+"')";
		    try{
      ...
      conn.commit();
      conn.executeUpdate("update topo_host_node a,nms_device_type b set a.type=b.descr where a.sys_oid=b.sys_oid");
      //加入网络设备IP的别名
   ...
   {    
	   ...
	   /*执行设备数据放入关系数据库的操作
		  * 例如：插入设备数据到mysql的topo_host_node表里面
		  * 需要一个到mysql(默认)数据库的连接
		  */
		 DBManager connForMysql = new DBManager();
	  for(int i=0;i<hostList.size();i++)
      {
		 try{
...
//			allipstr=ip1+ip2+ip3+ip4;
	     ...
	     connForMysql.addBatch(sql.toString());
	     	donehost.put(node.getId(), node);
	     	/*存放历史数据的带IP的历史表放入Hbase数据库，这些表需要动态的创建
	     	 * 设备数据存入对应的表中，这些表预先在关系数据库（mysql，oracle）创建好了
	     	 */
	     	//加入连通性检测类型表,若是PING检测则不加入
	     	if(node.getCollecttype() == SystemConstant.COLLECTTYPE_TELNETCONNECT||
					node.getCollecttype() == SystemConstant.COLLECTTYPE_SSHCONNECT){
...
//		conn.executeBatch();
		...
		connForMysql.addBatch("update topo_host_node a,nms_device_type b set a.type=b.descr where a.sys_oid=b.sys_oid");
      //加入网络设备IP的别名
      donehost = new Hashtable();
      for(int i=0;i<hostList.size();i++){
    	  try{
   ...
   public void addMonitor(int node_id,String ip,String category)
   {
	   ...
	   {	   		   
		   for(int i=0;i<moList.size();i++)
		   {
			   ...
			   nmID++;
			   StringBuffer sql = new StringBuffer(200);
			   sql.append("insert into topo_node_monitor(id,node_id,node_ip,category,moid,unit,threshold,compare,compare_type,upper_times,");
