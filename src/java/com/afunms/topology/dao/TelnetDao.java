...
public class TelnetDao extends BaseDao implements DaoInterface
{
   ...
   public boolean update(BaseVo baseVo)
   {
	   ...
	   sql.append("update server_telnet_config set user='");
   ...
   {
	   List list = findByCriteria("select * from server_telnet_config where node_id=" + nodeId);
	   if(list.size()>0)
   ...
   public BaseVo loadFromRS(ResultSet rs)
   {
	   ...
	   try
	   {
		   ...
		   vo.setUser(SysUtil.ifNull(rs.getString("user")));
