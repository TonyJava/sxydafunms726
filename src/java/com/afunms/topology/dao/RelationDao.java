...
public class RelationDao extends BaseDao implements DaoInterface {
	...
	/**
	 * 根据节点id 与roomid查找一条记录
	 */
	public BaseVo findByNodeandRoomid(String nodeId, String room_id) {
		BaseVo vo = null;
		try {
			rs = conn
					.executeQuery("select * from node_submap_relation where room_id='"
							+ room_id + "' and node_id='" + nodeId + "'");
			if (rs.next())
				vo = loadFromRS(rs);
		} catch (Exception ex) {
			SysLogger.error("RelationDao.findByNodeId()", ex);
	 ...
	 * 按map_id找一条记录
	 */
	public List<Relation> findByMapId(String mapId,String xmlname) {
		List<Relation> list = new ArrayList();
		Relation vo = null;
		try {
			rs = conn
					.executeQuery("select * from node_submap_relation where map_id='"
							+ mapId + "' or (xml_name='"+xmlname+"'  and map_id='-1')");
			while(rs.next()){
	...
	public BaseVo findByRoomNodeId(String nodeId, String xmlName,String roomid) {
		BaseVo vo = null;
		try {
			rs = conn
					.executeQuery("select * from node_submap_relation where xml_name='"
							+ xmlName + "' and node_id='" + nodeId + "'" + " and room_id='"+ roomid + "'");
			if (rs.next())
				vo = loadFromRoomRS(rs);
		} catch (Exception ex) {
			SysLogger.error("RelationDao.findByNodeId()", ex);
		}
		return vo;
	}
	public BaseVo loadFromRoomRS(ResultSet rs) {
		Relation vo = new Relation();
		try {
			vo.setId(rs.getInt("id"));
			vo.setXmlName(rs.getString("xml_name"));
	...
	public boolean updateRoom(BaseVo baseVo) {
		Relation vo = (Relation) baseVo;
		StringBuffer sql = new StringBuffer(200);
		sql.append("update node_submap_relation set xml_name='");
		sql.append(vo.getXmlName());
	...
	public boolean saveRoom(BaseVo baseVo) {
		Relation vo = (Relation) baseVo;
		StringBuffer sql = new StringBuffer(200);
		sql
				.append("insert into node_submap_relation(id,xml_name,node_id,category,map_id,room_id)values(");
		sql.append(getNextID());
		sql.append(",'");
		sql.append(vo.getXmlName());
		sql.append("','");
