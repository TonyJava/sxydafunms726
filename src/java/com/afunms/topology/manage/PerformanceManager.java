	...
	public String execute(String action)
	   {
		...
		System.out.println("=========PerformanceManager==action=="+action);
		    ...
		    public List monitorListSort(List montinorList ,  String category , String field , String type){
					for(int i = 0 ; i < montinorList.size() -1 ; i ++){
						for(int j = i + 1 ; j < montinorList.size() ; j ++){
							...
							if("name".equals(field)){
							...
							}else if ("swap".equals(field)){
								fieldValue = monitorNodeDTO.getSwapmemoryvalue();
								
								fieldValue2 = monitorNodeDTO2.getSwapmemoryvalue();
								...
								//if(null!=fieldValue && null!=fieldValue2)
								if("desc".equals(type)){
									// 如果是降序 则 前一个 小于 后一个 则交换
...
//		SysLogger.info("select * from topo_host_node where managed=1 "+s);
	...
	public MonitorNodeDTO getMonitorNodeDTOByHostNode(HostNode hostNode){
		...
		System.out.println("======首页ajax======="+hostNode.getIpAddress());
...
//		Hashtable eventListSummary = new Hashtable();
		...
		//System.out.println("==首页 node--Subtype="+dto.getType());
		
		
		//nodeutil.conversionToNodeDTO(host)
		
		
	    cpuValueColor=this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", dto.getType(), "cpu", ""));
			
		memoryValueColor=this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", dto.getType(), "physicalmemory", ""));
		    
		
		//System.out.println("====cpuValueColor="+cpuValueColor);
		//System.out.println("====memoryValueColor="+memoryValueColor);
...
//					System.out.println(where);
	...
	private String monitornodelist(){
		...
		System.out.println("==monitornodelist==="+monitornodelist.size());
		
        String jsp="";
        if(category.equals("net_server"))
        {
        	jsp="/performance/monitornodehostlist.jsp";
        }else
        {
		 jsp = "/performance/monitornodelist.jsp";
        }
		...
		if("1".equals(runmodel)){
			...
			List<NodeTemp> swapmemoryList = memoryInfoService.getSwapMemoryInfo(monitornodelist);
			//取出端口流速信息
			...
			for(int i=0; i<list.size(); i++){
				...
				//虚拟内存值
				if(swapmemoryList != null){
					for(int j=0; j<swapmemoryList.size(); j++){
						NodeTemp nodeTemp = swapmemoryList.get(j);
						if((monitorNodeDTO.getId()+"").equals(nodeTemp.getNodeid())){
							monitorNodeDTO.setSwapmemoryvalue(numberFormat.format(Double.parseDouble(nodeTemp.getThevalue())));
						}
					}
				}
		...
		String cpuValueColor = "";			// cpu 颜色
		String memoryValueColor = "";		// memory 颜色
		String swapmemorycolor="";    //swap 颜色
		for(int i=0; i<list.size(); i++){
			MonitorNodeDTO monitorNodeDTO = (MonitorNodeDTO)list.get(i);
			...
			if(memoryValueStr != null){
			 ...
			 if(category.equals("net_router")){
//				String physicalmemoryColor = AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "net", "physicalmemory", "");
//				String cpuColor = AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "net", "cpu", "");
//				String swapmemoryColor = AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "net", "swapmemory", "");
				cpuValueColor = this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "net", "cpu", ""));
				memoryValueColor = this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "net", "physicalmemory", ""));
				swapmemorycolor = this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "net", "swapmemory", ""));
				System.out.println("cpuValueColor:"+cpuValueColor+" memoryValueColor:"+memoryValueColor+" swapmemorycolor:"+swapmemorycolor);
			 }else{
//		        String physicalmemoryColor = AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "host", "physicalmemory", "");
//				String cpuColor = AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "host", "cpu", "");
//				String swapmemoryColor = AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "host", "swapmemory", "");
		    	cpuValueColor = this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "host", "cpu", ""));
				memoryValueColor = this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "host", "physicalmemory", ""));
				swapmemorycolor = this.computeColor(AgentalarmControlutil.GetAlarmlevel(monitorNodeDTO.getId()+"", "host", "swapmemory", ""));
				System.out.println("cpuValueColor:"+cpuValueColor+" memoryValueColor:"+memoryValueColor+" swapmemorycolor:"+swapmemorycolor);
			 }
...
//							cpuValueColor = "orange";red;yellow;green
...
//			}
			monitorNodeDTO.setCpuValueColor(cpuValueColor);
			monitorNodeDTO.setMemoryValueColor(memoryValueColor);
			monitorNodeDTO.setSwapmemorycolor(swapmemorycolor);
			//接口数量
...
//	    	    	try{
//	    	    		//获取被启用的所有被监视指标
//	    	    		monitorItemList = indicatorsdao.findByNodeIdAndTypeAndSubtype(host.getId()+"", "net", "");
//	    	    	}catch(Exception e){
//	    	    		e.printStackTrace();
//	    	    	}finally{
//	    	    		indicatorsdao.close();
...
//		  monitornodelist=null;
	...
	public String computeColor(String level){
		String color = "green";
		if(level.equals("1")){
			color="yellow";
