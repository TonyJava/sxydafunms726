...
public class PortScanDao extends BaseDao implements DaoInterface {
	...
	public Vector getCurrentStatus(String ip) {
		...
		try {
			...
			rs = conn
					.executeQuery("select p.thevalue value , s.name n,p.collecttime from nms_interface_data_temp  p ,system_portconfig s where p.ip=s.ipaddress and p.sindex=s.portindex and s.sms=1 and  p.subentity='ifOperStatus' and p.ip='"+ip+"'");
