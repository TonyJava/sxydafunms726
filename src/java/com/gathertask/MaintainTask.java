...
public class MaintainTask extends TimerTask{
	...
	public void run() {
		...
		if(null!= nlist && nlist.size()>0){
			...
			System.out.println("=====当前运行队列===="+nmsmemorydate.RunGatherLinst.toString());
...
//						Hashtable restable = ShareData.getResourceConfHashtable();//取出数据库资源维护任务数据
//						String res_id = gathertask.getId()+"";
//						Boolean flag = restable.containsKey(res_id);
						//jhl add
						if(ShareData.
								getResourceConfHashtable().
								containsKey(gathertask.getId()+""))
...
//						System.out.println("==任务配置被修改重启任务============="+gathertask.getId()+"");
				...
				}else{//内存中没有对应的定时任务ntl
					
					//jhl add
					if(!(ShareData.getResourceConfHashtable()).containsKey(gathertask.getId()+""))
					{
						tkmanager.createOneTask(gathertask);
					}
				 }
			}
			 System.out.println("=====开始迭代内存列表===="+nmsmemorydate.RunGatherLinst.size());
		 //迭代内存列表查找当前定时的任务
