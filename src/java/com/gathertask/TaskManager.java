 ...
 */
	 ...
	 */
		public void CreateMaintainTask()
		{
			...
			timer.schedule(btask, 1000, 2*1000*60);//按分钟执行定时任务
		 ...
		 */
			public void CreateDataTempTask()
			{
				...
				timer.schedule(btask, 20000, 30*1000);//按分钟执行定时任务
			 ...
			 */
				public void CreateGCTask()
				{
					...
					timer = new Timer();
					btask=new GcTask();
					timer.schedule(btask, 0, 1000*60);//按分钟执行定时任务
...
//				         System.out.println("=zz=="+nodeGatherIndicators.getId());
						...
						if(null!=nmsmemorydate.TaskList && nmsmemorydate.TaskList.size()>0&& nmsmemorydate.TaskList.containsKey(nodeGatherIndicators.getId()))
						{
							...
							nmsmemorydate.RunGatherLinst.remove(nodeGatherIndicators.getId());
							//jhl add 添加if判断
						}else if(!(ShareData.getResourceConfHashtable()).containsKey(nodeGatherIndicators.getId()+"")){//建立定时采集任务
							
//							System.out.println("=bb=="+nodeGatherIndicators.getId());
							timer = new Timer();
							btask=new BaskTask();
